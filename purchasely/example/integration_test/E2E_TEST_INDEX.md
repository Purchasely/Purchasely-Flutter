...
- ⚠️ **`appHandlesRedemptionAlert: true` is mandatory for R4, on every wrapper.** With the native default (`false`) the SDK shows its own popin and calls the listener only once the user **acknowledges** it, so a headless run gets an empty list and R4 fails — this cost three CI attempts on iOS before the cause was found. `true` suppresses the popin and delivers as soon as the redemption settles. Consequence: the **default popin path is not covered end to end** on any platform, because it is not observable without driving native UI.
- **Port:**
